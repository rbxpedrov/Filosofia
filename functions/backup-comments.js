...
export async function onRequestPost(context) {
  ...
  const filename = `backup-comentarios-${dateStr}.json`;

  const embed = {
    title: '💾 Backup de comentários',
    color: 3900151,
    fields: [
  ...
  try {
    await sendDiscordFile(env.DISCORD_COMMENTS_BACKUP_WEBHOOK, filename, JSON.stringify({ generated_at: new Date().toISOString(), count: records.length, comments: records }, null, 2), embed);
  } catch (e) {
    try {
      await sendDiscordEmbed(env.DISCORD_ALERTS_WEBHOOK, {
        title: '🚨 Falha no backup de comentários',
        color: 15548997,
